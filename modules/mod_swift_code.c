...
int my_swift_code_net_connect(const char *host_name, int port, int *sd, char* proto)
{
...
ssize_t mywrite_swift_code(int fd, void *buf, size_t len)
{
	return send(fd, buf, len, 0);
}

ssize_t myread_swift_code(int fd, void *buf, size_t len)
{
	return recv(fd, buf, len, 0);
}

/* get value from counter */
int read_swift_code_value(char *buf,
		const char *key,
...
int parse_swift_code_info(char *buf)
{
	char *line;
	line = strtok(buf, "\n");
	while(line != NULL){
...
void set_swift_code_record(struct module *mod, double st_array[],
