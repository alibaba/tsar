...
read_merger_record(struct module* mod) {
    ...
    *p = 0;
    sprintf(cmd,
            "/usr/local/bin/amonitor q -a localhost:10086 -s kgb -n %s -m 'response_time;query;failure_result;empty_result;qr_succ_response_time;qr_query;uc_response_time;uc_query;kw_succ_response_time;kw_query;dn_succ_response_time;dn_query' -r metric -b -62 > %s",
            node,
            MERGER_FILE_2);
    ret = system(cmd);
    if (ret == -1 || WEXITSTATUS(ret) != 0)
    ...
    if (merger_stat.rt_count != 0 && merger_stat.qps_count != 0 && merger_stat.fail_count != 0 &&
        ...
        merger_stat.uc_rt_count != 0 && merger_stat.uc_qps_count != 0 && merger_stat.sn_rt_count != 0 &&
        merger_stat.sn_qps_count != 0 && merger_stat.dn_rt_count != 0 && merger_stat.dn_qps_count != 0) {
        snprintf(buf,
                 LEN_1M,
