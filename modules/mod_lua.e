# 1 "mod_lua.c"
# 1 "/home/xiaokaikai.xk/work/tsar/modules//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "mod_lua.c"
...
struct module {
    ...
    unsigned long long *pre_array;
    unsigned long long *cur_array;
    double *st_array;
    double *max_array;
    double *mean_array;
    double *min_array;
    ...
    void (*set_st_record) (struct module *, double *, unsigned long long *, unsigned long long *, int);
...
struct configure {
    ...
    char output_interface[128];


    char output_print_mod[512];
    char output_stdio_mod[512];
    char output_nagios_mod[512];
    int print_interval;
    ...
    char output_db_mod[512];
    char output_db_addr[512];


    int output_tcp_addr_num;
    char output_tcp_mod[512];
    char output_tcp_addr[4][256];
    char output_tcp_merge[256];


    char server_addr[512];
    int server_port;
    int cycle_time;
    char send_nsca_cmd[512];
    char send_nsca_conf[512];

    char check_name[32][32];
    float wmin[32];
    float wmax[32];
    float cmin[32];
    float cmax[32];
...
void output_file();
# 42 "../include/tsar.h" 2
# 1 "../include/output_print.h" 1
# 27 "../include/output_print.h"
void running_print();
...
# 43 "../include/tsar.h" 2
# 1 "../include/output_db.h" 1
# 28 "../include/output_db.h"
...
# 29 "../include/output_db.h" 2


void output_db(int have_collect);
# 44 "../include/tsar.h" 2
# 1 "../include/output_tcp.h" 1
# 30 "../include/output_tcp.h"
void output_multi_tcp(int have_collect);
struct sockaddr_in * str2sa(char *);
# 45 "../include/tsar.h" 2
# 1 "../include/output_nagios.h" 1
# 31 "../include/output_nagios.h"
void output_nagios();
# 46 "../include/tsar.h" 2
# 1 "../include/common.h" 1
# 29 "../include/common.h"
int convert_record_to_array(unsigned long long *array, int l_array, const char *record);
void get_mod_hdr(char hdr[], const struct module *mod);
int strtok_next_item(char item[], char *record, int *start);
int merge_mult_item_to_array(unsigned long long *array, struct module *mod);
...
# 56 "/usr/local/include/lua.h" 3
typedef struct lua_State lua_State;
# 89 "/usr/local/include/lua.h" 3
...
# 11 "/usr/local/include/lualib.h" 3
# 1 "/usr/local/include/lua.h" 1 3
# 12 "/usr/local/include/lualib.h" 2 3
...
static struct mod_info lua_info[] = {
    ...
    {"luadat", DETAIL_BIT, MERGE_SUM, STATS_SUB},
};


static void
set_lua_record(struct module *mod, double st_array[],
    unsigned long long pre_array[], unsigned long long cur_array[], int inter)
{
    int i;
    st_array[0] = pre_array[0] + cur_array[0];
    return;
    for (i = 0; i < 3; i++) {
...
read_one_lua_stats(char *parameter, char * buf, int pos)
{
    ...
    luaL_loadfilex(L,parameter,((void *)0));

    lua_getglobal(L, "read_data");

    (((lua_type(L, (-1)) == 6)) ? (void) (0) : __assert_fail ("(lua_type(L, (-1)) == 6)", "mod_lua.c", 69, __PRETTY_FUNCTION__));

    printf("will call lua\n");
    if (lua_pcallk(L, (0), (1), (0), 0, ((void *)0)) != 0) {
        printf("Error running function f:%s`", lua_tolstring(L, (-1), ((void *)0)));
    }
    ...
    double c = lua_tonumberx(L,(-1),((void *)0));
    lua_settop(L, -(1)-1);
    ...
    if (write_flag) {
        pos += snprintf(buf + pos, 1048576 - pos, "lua1=%lld" ";",
                st_lua.luadat
                );
        if (strlen(buf) == 1048576 - 1) {
            return -1;
        }
        return pos;
    } else {
...
read_lua_stats(struct module *mod, char *parameter)
{
    ...
    char buf[1048576];
    char *token;
    char mod_parameter[256];
    char *lua_file_name = "/tmp/testlua.lua";

    buf[0] = '\0';

    pos = read_one_lua_stats(lua_file_name, buf, pos);
    return;


    strcpy(mod_parameter, parameter);
    if ((token = strtok(mod_parameter, " \t\r\n")) == ((void *)0)) {
